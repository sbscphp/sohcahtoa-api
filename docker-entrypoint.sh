...
if npx prisma migrate deploy 2>&1; then
  ...
  if check_failed_migration; then
    ...
    if npx prisma migrate deploy 2>&1 | grep -q "P3005"; then
      ...
      npx prisma migrate resolve --applied 20260615000000_add_change_password_otp_purpose 2>&1 || true
...
DO $$ BEGIN
  ALTER TYPE "DocumentType" ADD VALUE IF NOT EXISTS 'STUDENT_PASSPORT';
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
